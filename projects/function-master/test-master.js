(function(){
  ...
  QUnit.test("Variables created with var in a funtion are re-created each time", function(assert){
    function yay(){
      // var counter;
      if(this.counter !== undefined){ // false
        counter = counter + 1;
      } else {                        // true, so counter = 10
        var counter = 10;
      }
    }
  ...
  QUnit.test("We can pass functions to other functions and then run them.", function(assert){
    ...
    function something(whatever){   // use yay for parameter
      im_outter *= 20;              // outter im_outter is used, so 10 * 20 === 200
      whatever();                   // 200 / 5 === 40
    }
    something(yay);
  ...
  QUnit.test("We can get crazy with returns.", function(assert){
    ...
    function foo(whatever){               // use yay as parameter, resolves to " is dog"
      return "hello, this" + whatever();  // foo returns: "hello, this is dog"
    }
